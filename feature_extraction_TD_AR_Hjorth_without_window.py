...
def features_estimation(signal):
    """
    ...
    feat = np.zeros((1, Nsignals * NFPC))

    for i in range(Nsignals):
        td_matrix = time_features_estimation(signal[i, :])
        ar_matrix = autoregressive_coefficients(signal[i, :])
        hjorth_matrix = hjort_parameters(signal[i, :])
...
def time_features_estimation(x):
    """
    ...
    mav.append(np.sum(np.absolute(x)) / x.size)  # Mean Absolute Value
    ...
    dasdv.append(
        math.sqrt((1 / (x.size - 1)) * np.sum((np.diff(x)) ** 2)))  # Difference absolute standard deviation value
